...
/**
 * Plugin validation tests for SensorsPlus.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('khalidmaquilang/sensors-plus');
        expect($manifest['namespace'])->toBe('SensorsPlus');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/SensorsPlusFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.khalidmaquilang.plugins.sensors_plus');
        expect($content)->toContain('object SensorsPlusFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/SensorsPlusFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum SensorsPlusFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/SensorsPlusServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Khalidmaquilang\SensorsPlus');
        expect($content)->toContain('class SensorsPlusServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/SensorsPlus.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Khalidmaquilang\SensorsPlus\Facades');
        expect($content)->toContain('class SensorsPlus extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/SensorsPlus.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Khalidmaquilang\SensorsPlus');
        expect($content)->toContain('class SensorsPlus');
    });
});
